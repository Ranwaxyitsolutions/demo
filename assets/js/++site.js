$(document).ready(function() {
	...
	$('.go-next').click(function() {
		...
		var thiss = $(this);
		$('html, body').animate({
			scrollTop : $('.list-for-sale').offset().top - 90
		}, 700);
	});
	...
	$('.range-row .max-box ul li').on('click', function() {
		...
		if ($(this).index() == 0) {
			...
			$('.min-box').find('ul').find('li').eq($(this).index()).prevAll('li').show();
			$('.min-box').find('ul').find('li').eq($(this).index()).nextAll('li').hide();
		}
	...
	$('.tab-row a').click(function(e) {
		...
		if (datepickeranytimebool) {
			...
			if ($(this).hasClass("tab3 active")) {
				...
				$(".search-row-wrap.anytime span.dateinput-start").text("Any time");
	...
	$('.search-row-wrap.anytime').on('click', function() {

		//var date=     	$(".search-row-wrap.anytime").daterangepicker('setStartDate');
	...
	if ($(window).width() >= 768) {
		...
		$(".feature_section_list li").click(function() {
			$(".energy_balance_box").slideToggle();
		})
	}
	...
	$(".tab-content .clear-btn").click(function(event) {
		...
		$(this).parents(".tab-content").find(".slideOpen").find('input:checked').attr("checked", false);
		setupLabel();
		$(this).parents(".tab-content").find(".slideOpen").children().each(function() {
			$(this).find(".dropdown-value").text($(this).attr("default-text")).append("<em><b>&nbsp;</b></em>")
		});
	...
	try {
		...
		$(window).scroll(function() {
			...
			if ($(window).scrollTop() > barOffset) {
				$('.agent-form-fixed').addClass('show')
				if ($('.agent-form-fixed').hasClass('show') && $(".agent_form").css("display") == "block") {
				...
				$('.agent-form-fixed').removeClass('show');
				$('.agent_form-hd-fixed .agent_form').css("display", "block");
				temp = true;
			}
			if ($(window).scrollTop() < barOffset && contantOffset < $(window).scrollTop()) {
				$('.agent_form').slideDown(0);

			}
			// agent form fixed on window scroll in holiday html page
			if ($(window).scrollTop() > barOffset_sec) {
				var h3offset = $(".heading-row").offset().top - ($('.agent_form-hd-fixed').height() + barOffset_sec );
				$('.agent_form-hd-fixed').addClass('show');
				$('.agent_form-hd-fixed').removeClass('ksp');
				if ($(window).scrollTop() > (barOffset_sec + h3offset )) {
					$('.agent_form-hd-fixed').removeClass('show');
					$('.agent_form-hd-fixed').addClass('ksp');
...
// $('body').on('click', '.dateinput-start', function(){
// 	
	// if($('.dateinput-start').text() != "Any time")
	// $('.cross-btn-cal').css({"display": "block"});
// 	
// })
